...
def testing_from_tfrecord(modelName='Mobile', fileset='val', batch_size=100,
                          saver_dir=r'D:\MyProject\Tensorflow\workspace\models\Inception\2017-11-10_23-14-00'):
    height, width = 227,227
    with tf.Session() as sess:
        city_image_batch, city_label_batch = next_city_batch(model=fileset, num_epochs=1,
                                                             batch_size=batch_size, out_height=height,
                                                             out_width=width, n_classes=2,
                                                             isProcess=False)
        noncity_image_batch, noncity_label_batch = next_noncity_batch(model=fileset, num_epochs=1,
                                                                      batch_size=batch_size, out_height=height,
                                                                      out_width=width, n_classes=2,
                                                                      isProcess=False)
        ...
        while 1:
            try:
                ...
                break
            temp_time = time.time()
            score = MODEL.model.test_on_batch(image_batch, label_batch, sample_weight=None)
...
if __name__ == "__main__":
    ...
    saver_dir = r'D:\MyProject\Tensorflow\workspace\models\Alexnet\2018-01-05_00-11-35-CE'
    # testing(modelName='Inception', fileset='test', batch_size=100, saver_dir=saver_dir)
    testing_from_tfrecord(modelName='Alexnet', fileset='test', batch_size=100, saver_dir=saver_dir)